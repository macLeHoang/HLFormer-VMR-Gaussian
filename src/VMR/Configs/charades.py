...
cfg["hidden_size"]               = 288
cfg["n_heads"]                   = 6
cfg["num_queries"]               = 8            # Extra queries reduce decoder under-utilization.
cfg["dec_layers"]                = 3
cfg["input_drop"]                = 0.2
cfg["drop"]                      = 0.2
...
cfg["label_span_source"]         = "final"   # Options: coarse | refined | matched | final.
...
cfg["sal_prior_scale"] = 0.1  # Reduce early bias from the saliency prior.
...
cfg["boundary_refine_window"]          = 6
cfg["boundary_refine_learnable_sigma"] = True
cfg["boundary_refine_max_delta"]       = 0.02
...
# ---- Post-processing -----------------------------------------------------
cfg["top_k"]      = 8     # Keep one candidate per query.
cfg["nms_thresh"] = 0.45  # Slightly tighter NMS suits short Charades moments.

# ---- Optimizer -----------------------------------------------------------
cfg["lr"]                   = 1e-4
cfg["lr_vid_enc"]           = 0.5e-4
cfg["lr_refiner"]           = 1e-4   # Give the refinement head enough learning signal.
cfg["lr_txt_enc"]           = 0.5e-4
cfg["lr_drop"]              = 30        # Legacy field; unused when cosine_T0 > 0.
...
cfg["ema_decay"]  = 0.999
cfg["n_epoch"]    = 100
cfg["max_es_cnt"] = 6      # Stop after 6 validations without improvement.
cfg["batchsize"]  = 32
...
cfg["aug_schedule"] = [
    ...
    (15, {"temporal_crop_ratio": 0.2, "feat_mask_ratio": 0.10, "gt_jitter_frames": 1}),
    (30, {"temporal_crop_ratio": 0.1, "feat_mask_ratio": 0.05, "gt_jitter_frames": 0}),
]
...
# ---- Validation schedule -------------------------------------------------
cfg["val_freq"]       = 1
cfg["val_full_epoch"] = 10  # Run full validation from this epoch onward.
